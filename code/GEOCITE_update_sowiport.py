...
_search_body_title     = { 'query':     { 'match_phrase': { 'title': None } }, '_source':['id','title','corePublisher','publisher','type','coreAuthor'] };
_search_body_refstring = { 'query':     { 'multi_match':  { 'query': None, 'fields': ['title','author','coreJournalTitle','institutions','corePublisher'] } }, '_source':['id','title','corePublisher','publisher','type','coreAuthor'] };

#_scr_body = { "query": { 'match_all': {} } };
_scr_body = { "query": { "ids": { "values": _ids } } } if _ids else {'query':{'bool':{'must':{'term':{'has_gesis_ids': False}}}}} if not _recheck else {'query':{'match_all':{}}};
#-------------------------------------------------------------------------------------------------------------------------------------------------
#-FUNCTIONS---------------------------------------------------------------------------------------------------------------------------------------
...
def find(refobjects,client):
    ...
    for i in range(len(refobjects)):
        ...
        if doi != None:
            refobjects[i]['gesis_id'] = doi;
            ids.append(doi);
    return set(ids), refobjects;

def search_sowiport():
    client   = ES(['search.gesis.org/es-config/'],scheme='http',port=80,timeout=60);
    client_m = ES(['search.gesis.org/es-config/'],scheme='http',port=80,timeout=60);
    ...
    while returned > 0:
        for doc in page['hits']['hits']:
            ...
            body        = copy(_body);
            body['_id'] = doc['_id'];
            ids        = set(doc['_source']['gesis_ids']) if doc['_source']['gesis_ids'] != None else set([]);
            ...
            print('------------------------------------------------\n-- overall ids --------------------------------\n'+', '.join(ids)+'\n------------------------------------------------');
            body['_source']['doc']['gesis_ids']     = list(ids) if len(ids) > 0 else None;
            body['_source']['doc']['has_gesis_ids'] = True       if len(ids) > 0 else False;
            yield body;
        scroll_tries = 0;
...
i = 0;
for success, info in bulk(_client,search_sowiport(),chunk_size=_chunk_size, request_timeout=10):
    i += 1;
    if not success:
